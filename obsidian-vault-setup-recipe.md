...
A handful of folders, a few seed files, and a working "catch me up" command. In a terminal the script asks which folders you want -- keep, rename, or skip the defaults, then add your own -- or run it with `--defaults` to take the standard set. The whole thing is plain Markdown, so nothing here locks you into Obsidian or Claude. If you walk away from either tool, you still have a folder of notes.

---

## Two ways to do this

**The fast path:** run a script. Save the [setup script](#the-setup-script) below as `setup.sh` in your vault folder and run it -- or just tell Claude Code "run setup.sh for me." With no flags it sets up the folders and files (asking which folders if you run it in a terminal), which is all most people need. Flags add the optional tooling.
...
bash setup.sh --defaults      # accept the standard folder set, no questions
